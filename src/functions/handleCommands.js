    ...
    for (const path of folder) {
      ...
      if (isDir) {
        ...
        for (const subcommandFile of subcommandFiles) {
          try {
            const { default: command } = await import(
              `../commands/${path}/${subcommandFile}`
            );
            cmd.addSubcommand(command.data);

            console.log(
              `Command /${path} ${command.data.name} loaded successfully`,
            );
          } catch (error) {
            console.error(
              `Error loading command /${path} ${subcommandFile.replace(".js", "")}:`,
              error,
            );
          }
        }
        ...
        const execute = async (interaction) => {
          ...
          try {
            const { default: command } = await import(
              `../commands/${path}/${subcommand}.js`
            );

            await command.execute(interaction);
          } catch (error) {
            console.error(
              `Error executing command /${path} ${subcommand}:`,
              error,
            );
