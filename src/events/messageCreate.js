...
export default {
    ...
    async execute(message, client) {
        ...
        if (data.xp >= LEVELS[data.level + 1]) {
            ...
            if (guildData && guildData.level_roles && guildData.level_roles.get(data.level.toString())) {
                ...
                if (role) {
                    ...
                    if (member && !member.roles.cache.has(roleId)) {
                        try {
                            ...
                            console.error(`Failed to add role '${role.name}' to ${message.author.tag}:`, error);
                        }
                    }
                }
            }
